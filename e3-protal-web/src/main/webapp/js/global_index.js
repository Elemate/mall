	...
	return {
		...
		load: function(q) {
			...
			if( !options.url && options.matchContains ){
				...
				for( var k in data ){
					// don't content through the stMatchSets[""] (minChars: 0) cache
...
function getWordAll(isproduct,wwwurl){
	if(isproduct>0){
		var searchUrl = wwwurl + "/productlist/content?inputBox=1&keyword=";
		var Url = '/ajax/getWordAll/';
	}else{
		var searchUrl = wwwurl + "/article/content?keyword=";
...
function getkeyword(isproduct,wwwurl,formName){
	if(isproduct>0){
		...
		var str = '相关商品';
		var searchUrl = '/productlist/content';
		var typestr = '文章';
		var company1 = '篇';
		var company2 = '件';
	}
